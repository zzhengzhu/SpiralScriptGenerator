...
const SpiralText = ({
  ...
  const generateSpiralPath = (startRadius = 1, text, density = 0.1) => {
    ...
    const theta2 = calculateEndingAngle(
      textLength + offSet,
      startRadius,
      density
    );
    ...
    return {
      ...
      direction: { x: tangentX, y: tangentY },
    };
  };

  let { pathD, endPoint, direction } = generateSpiralPath(
    radius,
    text,
    density
  );
  ...
  return (
    <div>
      <svg id={"svgroot" + id} width="1600" height="1600" viewBox="-800 -800 1600 1600">
        <g
        ...
        >
          <path
            ...
            stroke="black"
          />
          <text className="hymmnos" style={{ fontSize: `${fontSize}px` }}>
